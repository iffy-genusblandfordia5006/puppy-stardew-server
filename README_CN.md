...
- 📦 **备份体验升级** - 备份改为后台任务，支持持久化进度状态，并可在面板中直接下载备份
- 🧩 **模组流程修复** - 自定义模组上传、删除和重启提示链路更加稳定
- 📋 **日志与仪表盘改进** - 分类日志更准确，服务器详情更完整，联机地址与运行状态显示更可靠
- ⚙️ **性能优化** - 增加低性能模式相关选项，并降低备份默认压缩级别，减少 CPU 峰值
- 🔒 **认证与持久化修复** - 面板认证数据持久化，首次密码设置流程更安全，运行时目录权限更稳定
